"""
Sistema Financiero - Home Depot Mexico Invoicing Portal
"""
import logging
...
class HomeDepotPortal(BasePortal):
    """Home Depot Mexico invoicing portal."""

    PORTAL_URL = "https://www.homedepot.com.mx/facturacion"
    MAX_RETRY_DAYS = 60

    def __init__(self):
        super().__init__("Home Depot", self.PORTAL_URL)
    ...
    async def login(self, page: Page, credentials: PortalCredentials) -> bool:
        try:
            ...
            logger.error(f"Home Depot login error: {e}", exc_info=True)
            return False
    ...
    async def request_invoice(self, page: Page, ticket_data: dict) -> bool:
        try:
            ...
            if ticket:
                inp = page.locator("input[name*='ticket'], input[name*='folio'], input[placeholder*='ticket']")
            ...
            logger.error(f"Home Depot invoice error: {e}", exc_info=True)
            return False
    ...
    async def download_files(self, page: Page) -> tuple:
        ...
        try:
            ...
            for ext, label in [(".pdf", "PDF"), (".xml", "XML")]:
                links = page.locator(f"a[href*='{ext}'], a:has-text('{label}'), button:has-text('{label}')")
            ...
            logger.warning(f"Home Depot download error: {e}")
