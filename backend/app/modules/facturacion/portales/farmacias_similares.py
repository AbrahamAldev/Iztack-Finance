"""
Sistema Financiero - Farmacias Similares Invoicing Portal
"""
import logging
...
class FarmaciasSimilaresPortal(BasePortal):
    """Farmacias Similares invoicing portal."""

    PORTAL_URL = "https://www.farmaciasimilares.com/facturacion"
    MAX_RETRY_DAYS = 30

    def __init__(self):
        super().__init__("Farmacias Similares", self.PORTAL_URL)
    ...
    async def login(self, page: Page, credentials: PortalCredentials) -> bool:
        try:
            ...
            logger.error(f"Farmacias Similares error: {e}", exc_info=True)
            return False
    ...
    async def request_invoice(self, page: Page, ticket_data: dict) -> bool:
        try:
            ...
            submit_btn = page.locator("button[type='submit'], button:has-text('Facturar'), button:has-text('Solicitar')")
            ...
            logger.error(f"Farmacias Similares invoice error: {e}", exc_info=True)
            return False
    ...
    async def download_files(self, page: Page) -> tuple:
        ...
        try:
            ...
            logger.warning(f"Farmacias Similares download error: {e}")
