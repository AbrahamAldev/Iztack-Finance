...
class BPPortal(BasePortal):
    """BP Mexico invoicing portal."""

    PORTAL_URL = "https://www.bp.com/mx/facturacion"
    MAX_RETRY_DAYS = 30

    def __init__(self):
        super().__init__("BP", self.PORTAL_URL)
    ...
    async def login(self, page: Page, credentials: PortalCredentials) -> bool:
        try:
            ...
            logger.error(f"BP error: {e}", exc_info=True)
            return False
    ...
    async def request_invoice(self, page: Page, ticket_data: dict) -> bool:
        try:
            ...
            logger.error(f"BP invoice error: {e}", exc_info=True)
            return False
    ...
    async def download_files(self, page: Page) -> tuple:
        ...
        try:
            ...
            logger.warning(f"BP download error: {e}")
