  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(data.error || "Error al procesar el ticket");
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        {mode === "select" && (
          <div className="p-6 space-y-4">
            <button
              onClick={startCamera}
              className="w-full flex items-center gap-3 p-4 border-2 border-dashed border-sky-300 rounded-xl hover:bg-sky-50 transition"
            >
              <Camera className="h-8 w-8 text-sky-600" />
              <div className="text-left">
                <p className="font-medium text-gray-900">Tomar foto</p>
            ...
            <button
              ...
              className="w-full flex items-center gap-3 p-4 border-2 border-dashed border-gray-300 rounded-xl hover:bg-gray-50 transition"
            >
              <ImageIcon className="h-8 w-8 text-gray-600" />
              <div className="text-left">
                <p className="font-medium text-gray-900">Seleccionar archivos</p>
            ...
            {images.length > 0 && (
              <div className="pt-4 border-t">
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-sky-600 text-white rounded-lg font-medium hover:bg-sky-700"
                >
                  Revisar y subir
                </button>
              </div>
            ...
            <div className="relative bg-black">
              ...
              <div className="absolute inset-0 pointer-events-none">
                <div className="h-full w-full flex items-center justify-center">
                  <div className="w-[85%] h-[70%] border-2 border-white/50 rounded-lg">
                    <div className="absolute top-2 left-4 text-xs text-white/70">
                  ...
                  {images.length > 1 && (
                    <>
                      <button
                        ...
                        className="absolute left-2 top-1/2 -translate-y-1/2 p-1 bg-black/40 text-white rounded-full disabled:opacity-30"
                      >
                      ...
                      <button
                        ...
                        className="absolute right-2 top-1/2 -translate-y-1/2 p-1 bg-black/40 text-white rounded-full disabled:opacity-30"
                      >
                    ...
                    <div key={i} className="relative flex-shrink-0">
                      <img
                        ...
                        className={`w-16 h-16 object-cover rounded-lg cursor-pointer ${
                          i === currentIndex ? "ring-2 ring-sky-500" : ""
                        }`}
                ...
                {result && (
                  <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg text-sm mb-4">
                    ...
                    {result.data?.store_name && (
                      <p>Tienda: {result.data.store_name}</p>
                    )}
                    {result.data?.total_amount && (
                      <p>Total: ${result.data.total_amount.toFixed(2)}</p>
                    )}
